"""Differential-Privacy obfuscation: DP-Pix and DP-Blur (Fan 2019 §3.3)."""
...
def laplace_scale(b: int, m: int, eps: float) -> float:
    """Fan §3.3: sensitivity of pixelization per b×b cell = 255·m/b²."""
    if b <= 0:
...
def dp_pix(
    ...
    """DP-Pix (Fan §3.3):
      1. b x b block average.
      2. Add Laplace noise with scale = 255 * m / (b^2 * eps).
      3. Nearest-neighbor upsample back to original size; clip to [0, 255].
    """
...
def dp_blur(
    ...
    """DP-Blur (Fan §3.3):
      1. Run DP-Pix with a small cell width b0 (internal sensitivity = 255 * m / b0^2).
      2. Upsample to ORIGINAL size; clip.
      3. Apply Gaussian blur with kernel k x k.
    """
    if k % 2 == 0:
