"""Training loop for CNN_ATT, matching McPherson §5.5 hyperparameters."""
...
def _build_optimizer(model: nn.Module, lr: float) -> torch.optim.Optimizer:
    # McPherson §5.5: SGD, lr=0.01, momentum=0.9, weight_decay=5e-4
...
def _build_scheduler(optim: torch.optim.Optimizer) -> torch.optim.lr_scheduler.LambdaLR:
    # Matches McPherson §5.5 lr_decay=1e-7: lr_t = lr_0 / (1 + 1e-7 * step).
...
def train_one_model(
    ...
    """Train one attacker model and return per-epoch history.

    model_name: "cnn_att" (default, McPherson baseline) or "resnet18" (stronger attacker).
    """
    ...
    for epoch in range(1, epochs + 1):
        model.train(True)
        # Accumulate loss as GPU tensor to avoid per-batch CUDA sync from .item().
